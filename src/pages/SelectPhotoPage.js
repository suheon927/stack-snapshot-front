...
const SelectPhotoPage = () => {
  ...
  //초기에 불러오는 사진들
    ...
    // PictureCompletedPage에서 넘겨주는 쿼리 파라미터 3개 frameid groupid date
    const frameid = searchParams.get('frameid');
    const groupid = searchParams.get("groupid");
    const date = searchParams.get("date");

    useEffect(() => {
      // PictureCompletedPage에서 넘겨주는 쿼리파라미터 frameid값으로 frame 선택
      setCurrnetFrame(FRAMES[`frame${frameid}`])

      // PictureCompletedPage에서 넘겨주는 쿼리파라미터 date, groupid값으로 file api 이용해서 이미지 불러오기
      setPhotos([
      `http://localhost:8080/api/file?date=`+date+`&type=original&groupid=`+groupid+`&index=0`,
      `http://localhost:8080/api/file?date=`+date+`&type=original&groupid=`+groupid+`&index=1`,
      `http://localhost:8080/api/file?date=`+date+`&type=original&groupid=`+groupid+`&index=2`,
      `http://localhost:8080/api/file?date=`+date+`&type=original&groupid=`+groupid+`&index=3`,
      `http://localhost:8080/api/file?date=`+date+`&type=original&groupid=`+groupid+`&index=4`,
      `http://localhost:8080/api/file?date=`+date+`&type=original&groupid=`+groupid+`&index=5`,
    ]);
  ...
  const togglePhoto =(index)=>{
    if(SelectedPhotos.indexOf(photos[index])!=-1){
      removePhoto(SelectedPhotos.indexOf(photos[index]));
    }
    else{
      addPhoto(index);
    }
    ...
    <div className="big-grid">
        ...
        <div className="grid-frame">
          ...
          {photos.map((value, index) => {
              return ( 
                <img key={index} onClick={()=>{togglePhoto(index)}} className={`photo ${index} ${SelectedPhotos.includes(value) ? "selected-photo" : ""}`} src={value}  onError={(err)=>{console.log(170,err);}}/>
                );
              })}
          </div>
        </div>
        ...
        {<>
          {[...Array(currnetFrame["maxCount"]).keys()].map((value, index) => {
              return (
                <img key={index} className={`frame frame${currnetFrame["frameID"]} picture picture${index+1}`} src={SelectedPhotos[value]} alt="logo image" onClick={()=>{removePhoto(index)}}></img>
                );
            })}
        </>}
